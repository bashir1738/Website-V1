...
exports.update = async (req, res) => {
  try {
    const reg = await ProdfestRegistration.findByPk(req.params.id);
    ...
    console.error('Update prodfest registration error:', err);
    return res.status(500).json({ success: false, error: 'Server error' });
  }
};
...
exports.remove = async (req, res) => {
  try {
    ...
    console.error('Delete prodfest registration error:', err);
    return res.status(500).json({ success: false, error: 'Server error' });
  }
};
